...
Строка фильтра над списком эндпоинтов — маленький логический язык. Экспорт
(**Swagger** / **JSON** / **MD**) и кнопка «Домены» учитывают текущий фильтр.

**Поля** (`поле оператор значение`):

| Поле | Что сравнивает |
|------|----------------|
| `domain` / `host` | хост эндпоинта (в т.ч. `?` — неизвестный) |
| `path` | путь (`/api/v1/x`) |
| `url` | полный URL |
| `method` | HTTP-метод (`GET`, `POST`…) |
| `scheme` | `http` / `https` |
| `type` | тип, как его определил jsluice |
| `category` | `api` / `page` / `static` |
| `tag` | gf-тег (`redirect`, `lfi`, `ssrf`, `idor`…) |
| `param` | имя любого query/body-параметра |

**Операторы:**

| Оператор | Значение |
|----------|----------|
| `=` | равно (с поддержкой `*` как wildcard, регистр не важен) |
| `!=` | не равно |
| `~` | содержит подстроку |

**Булевы флаги** (пишутся без оператора): `has_param`, `api`, `static`.

**Логика:** `&&` (И), `||` (ИЛИ), `!` (НЕ), скобки `( )`. Значения поддерживают
`*` как любую последовательность символов.

**Примеры:**

```text
domain=*yoomoney.ru && method=GET && has_param
(category=api || has_param) && !domain=*sentry*
path~/admin && method!=GET
tag=redirect || tag=ssrf
param=redirect_url          # эндпоинты с параметром redirect_url
domain=?                    # только эндпоинты с неизвестным хостом
!static && method=POST
scheme=http                 # незашифрованные вызовы
url~/graphql
```
